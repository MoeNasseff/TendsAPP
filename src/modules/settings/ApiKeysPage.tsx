...
/**
 * Clone of https://demo.tailadmin.com/api-keys, transcribed from
 * `assets/re-desgin/tailadmin-pro-reference/api-keys.html` (lines 2880-3638).
 *
 * Every class, element, SVG and label is the reference's. Alpine translated as
 * usual: `x-data="{isModalOpen}"` → useState, `x-show` → conditional render,
 * `@click.outside` → the codebase's pointerdown pattern, `:class` → the same
 * ternary inline.
 *
 * Wired to real data: rows are this account's AI provider configs
 * (`ai_provider_configs` via useAIProviders), not the reference's three
 * hardcoded keys.
 *
 * On masking — the reference and this app happen to agree. Its inputs hold a
 * pre-masked literal (`sk_live_**********4248`) and it ships no reveal
 * control, only Copy. Here the mask is not cosmetic: `api_key` is excluded
 * from the authenticated role's column grants, so the browser cannot read a
 * key back even if it wanted to. Copy therefore copies the mask, not a secret
 * — see the note on that button.
 *
 * Divergences, all forced by that:
 *   - "Last used" reports an em dash; nothing records it yet.
 *   - Copy and Regenerate are inert (a key is not readable, and rotating one
 *     is the provider's job, not ours).
 *   - The modal's "application name" field became the key field, since that is
 *     what this page exists to capture.
 */

const CELL = 'px-5 py-3 whitespace-nowrap'
const HEAD = 'px-5 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400'
const KEY_INPUT =
        ...
        <div className="rounded-2xl border border-gray-200 bg-white px-6 pl-5 dark:border-gray-800 dark:bg-white/3">
          <div className="flex flex-col justify-between gap-5 border-b border-gray-100 py-4 sm:flex-row sm:items-center dark:border-gray-800">
            ...
            <div>
              <div>
                <button
                  className="bg-brand-500 hover:bg-brand-600 inline-flex items-center justify-center gap-2 rounded-lg px-4 py-3 text-sm font-medium text-white transition"
                    ...
                    <div className="relative w-full max-w-[600px] rounded-3xl bg-white p-6 lg:p-10 dark:bg-gray-900">
                      ...
                      <div>
                        ...
                        <p className="mb-7 text-sm leading-6 text-gray-500 dark:text-gray-400">
                          Paste a key from your provider. It is stored write-only — Tend can check
                          that a key exists, but can never read it back, and neither can this page.
                        </p>
                        <form
                          onSubmit={async (e) => {
                            e.preventDefault()
                        ...
                        >
                          <div>
                            ...
                            <select
                              ...
                              className="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 mb-4 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"
                            >
                              {providers.map((p) => (
                                <option key={p.id} value={p.id} className="text-gray-700 dark:bg-gray-900 dark:text-gray-400">
                                  {p.label}
                                </option>
                              ))}
                            </select>
                          ...
                          <div className="mt-8 flex w-full items-center justify-between gap-3">
                            <button
                              onClick={() => setIsModalOpen(false)}
                              type="button"
                              className="shadow-theme-xs s flex w-full justify-center rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200"
                            >
                              Close
                            </button>
                            <button
            ...
            <table className="min-w-full">
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-800">
                  ...
                  <th className={HEAD}>Disable/Enable</th>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {!loading &&
                  providers.map((provider) => {
                    ...
                    const configured = !!state?.hasKey
                    const enabled = !!state?.enabled && configured
                    const inputId = `api-${provider.id}`

                    return (
                      <tr key={provider.id}>
                        <td className="py-3 pr-5 whitespace-nowrap">
                          <div>
                            <label htmlFor={inputId} className="mb-2 inline-block text-sm text-gray-700 dark:text-gray-400">
                            ...
                            <div className="flex items-center gap-3">
                              <div className="relative">
                                <input
                                  readOnly
                                  value={configured ? `${provider.id}_**********` : 'Not configured'}
                                  id={inputId}
                                  className={KEY_INPUT}
                                />
                                {/* Inert by design. The value beside it is a
                                    mask, not the key — copying it would hand
                                    the user a string that authenticates
                                    nothing. Kept for visual parity. */}
                                <button
                                  type="button"
                                  disabled
                        ...
                        <td className={CELL}>
                          <span
                            className={
                              enabled
                                ? 'inline-flex items-center justify-center gap-1 rounded-full bg-green-50 px-2.5 py-0.5 text-xs font-medium text-green-600 dark:bg-green-500/15 dark:text-green-500'
                                : 'bg-error-50 text-error-600 dark:bg-error-500/15 dark:text-error-500 inline-flex items-center justify-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium'
                            }
                          >
                          ...
                          <label htmlFor={`toggle-${provider.id}`} className="cursor-pointer">
                            <div className="relative">
                              <input
                                ...
                                checked={enabled}
                                disabled={!configured}
                                onChange={() => setEnabled(provider.id, !enabled)}
                              />
                              <div
                                className={`block h-6 w-11 rounded-full ${
                                  enabled ? 'bg-brand-500 dark:bg-brand-500' : 'bg-gray-200 dark:bg-white/10'
                                }`}
                              ></div>
                              <div
                        ...
                        <td className={CELL}>
                          <div className="flex w-full items-center gap-3">
                            <button
                              ...
                              className="hover:text-error-500 dark:hover:text-error-500 text-gray-500 disabled:opacity-40 dark:text-gray-400"
                            >
